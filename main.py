...
def naming(message):
    ...
    bot.send_message(message.chat.id, 'Отправь аудиофайл в формате MP3')
    bot.register_next_step_handler(message, save_audio)  # Теперь save_audio определена выше
...
def save_audio(message):
    ...
    try:
        ...
        if not message.audio.file_name.endswith('.mp3'):
            bot.send_message(message.chat.id, 'Пожалуйста, отправьте файл в формате MP3')
            return
        ...
        file_path = MUSIC_DIR / f"{name}.mp3"
        ...
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton('Весь плейлист', callback_data='loadings'))
        bot.send_message(message.chat.id, '✅ Трек успешно добавлен!', reply_markup=markup)
